...
class PlanningAgent(OpenAIAgent):
    ...
    def _handle_response(self, response) -> None:
        ...
        if planning_response.type == "plan":
            ...
            # Ensure current_plan is always a list for safe iteration
            if isinstance(planning_response.content, list):
                self.current_plan = planning_response.content
            else:
                # Defensive: wrap string in list (shouldn't happen with proper LLM)
                logger.warning(f"Plan content was string, wrapping in list: {planning_response.content}")
                self.current_plan = [planning_response.content]
            ...
            if isinstance(response, PlanningAgentResponse):
                if response.type == "plan":
                    ...
                    if isinstance(response.content, list):
                        ...
                        return [
                            response.content
                        ]  # Wrap single dialogue message in a list
            else:
