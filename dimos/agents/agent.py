...
class LLMAgent(Agent):
    ...
    def __init__(
        ...
        self.input_query_stream = (
            ...
            if (input_data_stream is None)
                ...
                input_query_stream.pipe(
                    ...
                    RxOps.do_action(
                        lambda x: print(
                            f"\033[34mEnriched query: {x.split(chr(10))[0]}\033[0m"
                        )
                        or [
                            print(f"\033[34m{line}\033[0m")
                            for line in x.split(chr(10))[1:]
                        ]
                    ),
                )
            )
        ...
        if (self.input_video_stream is not None) and (
            self.input_query_stream is not None
        ):
    ...
    def _handle_tooling(self, response_message, messages):
        ...
        def _tooling_callback(
            message, messages, response_message, skill_library: SkillLibrary
        ):
...
class OpenAIAgent(LLMAgent):
    ...
    def __init__(
        ...
        skills: Optional[
            Union[AbstractSkill, list[AbstractSkill], SkillLibrary]
        ] = None,
        ...
        self.response_model = (
            response_model if response_model is not None else NOT_GIVEN
        )
