...
class UnitreeROSControl(ROSControl):
    ...
    CAMERA_TOPICS = {
        ...
        "info": {"topic": "camera/camera_info", "type": CameraInfo},
    }
    ...
    def _update_mode(self, msg: Go2State):
        ...
        Logic:
        - If progress is 0, robot is IDLE (ready for commands)
        - If progress is 1, robot is MOVING (executing a command)

        Note: mode field indicates robot's control mode (1=normal, 9=sport mode, etc.)
        but progress is the key indicator of whether robot is busy executing commands.
        """
        ...
        if progress == 0:
            ...
            logger.debug(f"Robot mode set to IDLE (progress=0, mode={mode})")
        elif progress == 1:
            self._mode = RobotMode.MOVING
            logger.debug(f"Robot mode set to MOVING (progress=1, mode={mode})")
        else:
